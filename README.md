...
`/tools` 提供 AI 中英文沟通、贷款与房贷计算、单位换算、共享账单分摊、租房费用计算和搬家清单。工具由 `?tool=communication|loan|units|split|budget|moving` 选择，入口在侧栏、手机顶部和快速搜索；同页切换保留输入，离开页面后计算和沟通输入不保留。

计算在浏览器中进行，不发送数值到 API。分摊内部按整数美分和最大余数法处理，保证合计等于总金额；租房费用明确区分月支出、一次性费用与押金现金占用。换算使用美制液体加仑，结果最多显示四位小数。

贷款计算支持房价与首付或直接本金、固定年利率、1–50 年期限，以及从首期开始的每月额外还本金。常规贷款本息与自填房产税、房屋保险、HOA、贷款保险分别展示；年度余额曲线与明细、提前还清时间、节省利息和复制摘要使用同一整数美分还款表。按月计息、逐期取整，末期余额调整会明确显示，不适用于浮动利率、仅付息、气球贷或按日计息产品。示例不是实时利率或税率；税费变化、成交费用与提前还款罚金不在估算内。
