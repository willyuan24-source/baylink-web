...
`/tools` 提供 AI 中英文沟通、单位换算、共享账单分摊、租房费用计算和搬家清单。工具由 `?tool=communication|units|split|budget|moving` 选择，入口在侧栏、手机顶部和快速搜索；同页切换保留输入，离开页面后计算和沟通输入不保留。

计算在浏览器中进行，不发送数值到 API。分摊内部按整数美分和最大余数法处理，保证合计等于总金额；租房费用明确区分月支出、一次性费用与押金现金占用。换算使用美制液体加仑，结果最多显示四位小数。

搬家清单按账号或访客保存于当前浏览器，版本化校验并限制自定义条数；损坏数据与存储失败会提示，不静默覆盖。AI 沟通只在点击生成后调用现有 `/ai/guide-chat`；取消、输入变化和账号切换会中止过期响应。仅真正 AI 模式返回作为可编辑草稿，降级模板不当作生成成功；生成草稿由用户核对并自行发送，网址需在生成后补入。
