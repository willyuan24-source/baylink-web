...
import { ImportedEventsPanel } from '../components/ImportedEventsPanel';
...
export default function MyWeekPage() {
  ...
  const thisWeek = MONTHLY_EVENTS.filter(event => (!preferences.regions.length || preferences.regions.includes(event.region)) && (!preferences.interests.length || preferences.interests.includes(event.category)))
    .flatMap(event => { const date = nextConfirmedEventDate(event, today); return date && date <= weekEnd ? [{ event, date }] : []; })
    .sort((a, b) => a.date.localeCompare(b.date) || a.event.id.localeCompare(b.event.id)).slice(0, 6);
  ...
  return <div className="planner-page week-page"><header className="planner-hero"><div className="planner-eyebrow"><CalendarDays size={16} /> YOUR WEEK, AT YOUR PACE</div><h1>我的这周，<br /><em>留给喜欢的事。</em></h1><p>{today} — {weekEnd}</p><nav><Link to="/plan">新建出游计划 <ArrowRight size={16} /></Link><Link to="/ai-in-the-bay">看看湾区 AI 活动 ↗</Link></nav></header>
    ...
    <ImportedEventsPanel userId={app?.user?.id} />
    {library.loading ? <p role="status" className="planner-note">正在读取你的计划…</p> : <>
      ...
      <section className="week-events"><div className="planner-section-head"><div><span className="planner-eyebrow">THE NEXT SEVEN DAYS</span><h2>这周可以去</h2></div><Link to="/calendar">整月日历 ↗</Link></div>{!thisWeek.length && <p className="planner-note">未来七天暂无符合偏好的已收录活动，可以放宽地区或兴趣。</p>}<div className="week-plan-grid">{thisWeek.map(({ event, date }) => <article className="week-event-card" key={event.id}><span>{event.dateLabel}</span><h3><Link to={`/events/${event.id}`}>{event.title}</Link></h3><p><MapPin size={14} /> {event.city} · {event.costLabel}</p><div><Link to={`/plan?stops=event:${event.id}&date=${date}`}>放进计划</Link><button onClick={() => downloadEventCalendar(event)}>存入日历</button></div></article>)}</div></section>
