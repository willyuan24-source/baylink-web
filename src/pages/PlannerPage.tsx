...
export default function PlannerPage() {
  ...
  const recommend = async (replace = false) => {
    ...
    try {
      ...
      if (request.current === controller && !controller.signal.aborted) setResults(result);
  ...
  const share = async () => {
    try { await navigator.clipboard.writeText(sharePlanUrl({ date, stops }, window.location.origin)); setStatus('公开地点与日期链接已复制；不包含账号或私人标题。'); } catch { setStatus('复制失败，请使用下方分享链接。'); }
  };
  ...
  return <div className="planner-page">
    ...
    <div className="planner-workspace"><main>
      ...
      {stops.length > 0 && <><button className="planner-share" onClick={() => void share()}><Share2 size={15} /> 分享地点与日期</button><a className="planner-public-link" href={sharePlanUrl({ date, stops })}>打开公开分享链接 ↗</a></>}
