...
export default function PlannerPage() {
  ...
  return <div className="planner-page">
    ...
    <div className="planner-workspace"><main>
      ...
      <div className="planner-catalog">{[...events.map(event => ({ kind: 'event' as const, id: event.id, title: event.title, city: event.city, summary: event.summary, label: `${translateText(event.dateLabel, locale)} · ${translateText(event.costLabel, locale)}`, point: event.location, official: event.officialUrl, image: GUIDE_IMAGES[event.imageKey] })), ...filteredPlaces.map(place => ({ kind: 'place' as const, id: place.id, title: place.title, city: place.city, summary: place.summary, label: '常设景点 · 开放时间请查官方', point: place.location, official: place.officialUrl, image: (() => { const guide = getGuideBySlug(place.guideSlug); return guide ? getGuideMedia(guide).cover : undefined; })() }))].map(item => { const key = `${item.kind}:${item.id}`; const pin = points.findIndex(point => point.key === key); return <article id={`catalog-${key}`} key={key} className={`planner-place ${selected === key ? 'is-selected' : ''}`}><div className="planner-place-heading"><button className="planner-place-title" onClick={() => setSelected(key)}>{pin >= 0 && <span>{pin + 1}</span>}<h3>{item.title}</h3></button><button aria-label={translateText(favorite(item.kind, item.id) ? '取消收藏' : '收藏到我的这周', locale)} aria-pressed={favorite(item.kind, item.id)} disabled={library.loading || library.busy} onClick={() => void library.toggleFavorite({ kind: item.kind, id: item.id })}><Heart size={17} fill={favorite(item.kind, item.id) ? 'currentColor' : 'none'} /></button></div>{selected === key && item.image && <div className="planner-place-media"><GuideFigure image={item.image} variant="preview" /></div>}<p className="planner-meta">{item.city} · {item.label}</p><p>{item.summary}</p><div className="planner-place-actions"><Link to={stopPath(item)}>阅读详情</Link>{item.point && <> <a href={item.point.sourceUrl} target="_blank" rel="noreferrer">坐标来源 ↗</a><a href={`https://www.google.com/maps/search/?api=1&query=${item.point.lat},${item.point.lng}`} target="_blank" rel="noreferrer">查看地图与路线 ↗</a></>}<button onClick={() => add({ kind: item.kind, id: item.id })}><Plus size={15} /> 加入计划</button></div></article>; })}</div>
    </main><aside id="outing-plan" className="planner-editor"><span className="planner-eyebrow">YOUR LITTLE ESCAPE</span><h2>排好这一天</h2><p>最多三站，留些时间给路上。</p><label>计划名称<input maxLength={80} value={title} onChange={e => setTitle(e.target.value)} /></label><label>日期<input type="date" value={date} onChange={e => setDate(e.target.value)} /></label>
