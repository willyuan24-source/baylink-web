  ...
  return <div className="lb-page">
    ...
    {region === 'sf' && !listMode ? <Suspense fallback={<div className="lb-world lb-loading"><Loader2 className="animate-spin" size={22} /><span>{t('正在走进迷你旧金山…', 'Entering little San Francisco…')}</span></div>}><SanFranciscoExplorer date={date} stops={stops} onAddPlace={addWorldPlace} onShowList={() => setListMode(true)} freeOnly={freeOnly} ownerId={app?.user?.id} /></Suspense> : <div className={`lb-world-layout ${listMode ? 'is-list' : ''}`}>
      ...
      <aside className="lb-stop-card" aria-label={t('下一站详情', 'Your next stop')}>
        ...
        {active ? <><div className={`lb-stop-image lb-stop-image--${active.kind}`}>{active.image ? <img key={active.image} src={active.image} alt={editorial(active.imageMeta?.alt || active.title)} style={active.imageMeta?.fullFrame || active.imageMeta?.kind === 'poster' ? { objectFit: 'contain' } : undefined} onError={event => { event.currentTarget.style.visibility = 'hidden'; }} /> : <><Compass size={58} strokeWidth={1} /><span>{active.city}</span></>}<span className="lb-image-badge">{active.kind === 'event' ? t('当天活动', 'ON THIS DAY') : t('湾区好去处', 'A BAY AREA FAVORITE')}</span></div>
