  ...
  return (
    <div className="px-4 pt-1 sm:px-5 sm:pt-2 pb-40 lg:pb-8 max-w-full overflow-x-hidden">
        <div className="relative mb-1 sm:mb-2 group">
          ...
          <input aria-label="搜索本地信息" type="search" className="search-input" placeholder="搜索房源、服务、二手、接送..." value={keyword} onChange={e => setKeyword(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.nativeEvent.isComposing && searchPostsNow()} />
        </div>
        ...
        {!keyword && !categorySlug && (
          <>
            ...
            />
            <details id="baybay-home-entry" className="mb-3 rounded-xl border border-baylink-border p-3">
              <summary className="cursor-pointer text-sm font-medium">需要找信息或整理发帖内容？问问 BayBay</summary>
        ...
        {categorySlug && <h1 className="mb-3 text-xl font-bold">{categoryFilter}</h1>}
        {categoryFilter === '本地服务' && <div className="mb-3 flex flex-wrap gap-2">{SERVICE_CATEGORIES.map(c => <CategoryChip key={c} label={c} active={false} onClick={() => navigateToCategory(c)} />)}</div>}
        ...
        <div className="flex items-center justify-between mb-2 px-0.5">
          <h2 className="text-sm font-semibold text-baylink-text">最新发布</h2>
          <span className="text-[11px] text-baylink-muted">{feedType === 'provider' ? '本地资源' : '邻里需求'}</span>
        </div>

        {feedError && posts.length > 0 && <div role="status" className="mb-3 rounded-lg bg-amber-50 p-3 text-sm text-amber-900"><p>更新失败，当前保留已加载信息。</p><button type="button" onClick={retryFeed} className="mt-2 font-semibold underline">重新加载当前结果</button></div>}
        {feedError && posts.length === 0 && !isInitialLoading ? (
          ...
          <div>
            ...
            <p className="pb-3 pt-1 text-center text-[11px] text-baylink-muted">首次加载可能需要几秒钟，请稍候。</p>
          </div>
        ) : posts.length === 0 ? (
          <><EmptyFeed
            feedType={feedType}
          ...
          />{hasMore && <button type="button" onClick={handleLoadMore} disabled={isLoadingMore} className="btn-primary mx-auto mb-6 block px-5 py-3">{isLoadingMore ? '继续查找中…' : '继续查找更多信息'}</button>}</>
        ) : (
          <>
        ...
        {!keyword && !categorySlug && <HotRecommend onOpenPost={navigateToPost} refreshKey={featuredRefreshKey} onViewMore={() => navigate('/recommend')} />}
    </div>
  );
}
