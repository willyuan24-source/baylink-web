import { getLocale, localizedUrl, translateText } from '../i18n/locale';
...
const line = (label: string, value?: string) => value?.trim()
  ? `${translateText(label)}${getLocale() === 'en' ? ': ' : '：'}${value.trim()}` : '';

export const buildPostShareUrl = (post: Pick<ShareablePost, 'id'>): string => {
  return localizedUrl(`${SITE_URL}/posts/${encodeURIComponent(post.id)}`);
};

export const buildPostShareText = (post: ShareablePost): string => {
  const url = buildPostShareUrl(post);
  const area = post.city?.trim() || translateText('湾区');
  const title = post.title?.trim() || translateText('本地信息');
  ...
  if (RENT_CATEGORIES.has(category)) {
    return [
      translateText('我在 BAYLINK 看到一个湾区租房信息：'),
      '',
      `【${title}】`,
      ...
      url,
      '',
      translateText('BAYLINK｜湾区生活信息站'),
    ].join('\n');
  }

  if (USED_CATEGORIES.has(category)) {
    return [
      translateText('我在 BAYLINK 看到一个湾区二手信息：'),
      '',
      `【${title}】`,
  ...
  if (SERVICE_CATEGORIES.has(category)) {
    return [
      translateText('我在 BAYLINK 看到一个湾区本地服务信息：'),
      '',
      `【${title}】`,
  ...
  return [
    translateText('我在 BAYLINK 看到一条湾区本地信息：'),
    '',
    `【${title}】`,
...
const copyText = async (text: string): Promise<boolean> => {
  try {
    ...
    try {
      ...
      ta?.remove();
      if (typeof HTMLElement !== 'undefined' && focused instanceof HTMLElement && focused.isConnected) focused.focus({ preventScroll: true });
    }
  }
};
...
export async function sharePost(
  ...
  const url = buildPostShareUrl(post);
  const title = post.title?.trim() || translateText('BAYLINK 帖子');
