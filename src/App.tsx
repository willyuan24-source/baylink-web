...
const LoginModal = ({ onClose, onLogin }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch (err: any) { setError(err.message || '操作失败，请重试'); } finally { setLoading(false); }
  };

  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-6 z-[60] backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-xs relative transform transition-all">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {isRegister && (
            <>
              <input required className="w-full p-3 bg-gray-50 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500" value={form.nickname} onChange={e => setForm({...form, nickname: e.target.value})} placeholder="你的昵称" />
...
const ChatView = ({ currentUser, conversation, onClose }: any) => {
  ...
  return (
    <div className="fixed inset-0 bg-white z-[100] flex flex-col">
      ...
      <div className="p-3 bg-white border-t flex gap-2 items-center pb-safe">
    ...
    <div className="fixed inset-0 bg-[#F3F4F6] flex justify-center font-sans text-gray-900"> {/* 使用 fixed inset-0 确保占满手机全屏且不滚动 */}
      <div className="w-full max-w-[480px] bg-white h-full shadow-2xl relative flex flex-col border-x border-gray-200"> {/* h-full 继承父级高度 */}
        ...
        <main className="flex-1 overflow-y-auto bg-[#FAFAFA] hide-scrollbar">
            ...
            {tab === 'messages' && (
              <div className="p-4 pt-safe-top pb-24">
                <h2 className="text-xl font-bold mb-4 px-2">消息列表</h2>
                {!user ? <div className="text-center text-gray-400 text-sm mt-10">请先登录查看消息</div> : 
                myConvs.length > 0 ? myConvs.map(c => (
                  <div key={c.id} onClick={() => setChatConv(c)} className="flex items-center gap-3 p-4 hover:bg-gray-50 rounded-2xl cursor-pointer mb-3 border border-gray-100 bg-white shadow-sm">
                    <div className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center text-blue-600 font-bold text-lg">{c.otherUser.nickname?.[0]}</div>
                    <div><div className="font-bold text-sm text-gray-900">{c.otherUser.nickname}</div><div className="text-xs text-gray-500 truncate w-48 mt-1">{c.lastMessage || '点击查看消息'}</div></div>
                  </div>
