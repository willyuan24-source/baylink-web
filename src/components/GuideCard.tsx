...
export const GuideCard = ({ guide, onClick, compact }: GuideCardProps) => (
  <button
    type="button"
    onClick={onClick}
    className={`flex w-full min-w-0 cursor-pointer flex-col rounded-2xl border border-baylink-border/50 bg-white text-left shadow-card transition hover:border-baylink-green/30 hover:bg-gray-50/80 active:scale-[0.99] ${
      compact ? 'p-3' : 'p-4'
    }`}
  >
