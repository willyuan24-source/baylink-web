...
function DiscoveryArticle({ item, today }: { item: LocalDiscovery; today: string }) {
  ...
  return <article className="local-discovery-detail">
    ...
    <div className="discovery-detail-body">
      ...
      <div className="discovery-detail-links"><a className="discovery-primary" onClick={() => recordProductEvent('official_source_click')} href={officialUrl} target="_blank" rel="noopener noreferrer">{item.kind === 'offer' ? '查看领取入口' : item.kind === 'opening' ? '查看商家官网' : '查看主办方详情'}<ArrowUpRight size={17} /></a>{item.kind === 'event' && !ended && <button type="button" onClick={() => downloadEventCalendar(item.event)}><CalendarDays size={17} />存入日历</button>}{item.kind === 'offer' && item.offer.storeUrl && <a href={item.offer.storeUrl} target="_blank" rel="noopener noreferrer">查询本地门店<ArrowUpRight size={17} /></a>}</div>
