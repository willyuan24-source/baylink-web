...
export function RentalBudgetTool({ onToast }: { onToast: ShowToast }) {
  ...
  return <div className="tool-form">
    ...
    <div className="tool-actions"><button type="button" className="tool-button" disabled={!result} onClick={() => copyResult(summary, onToast)}><Copy size={16} />复制费用摘要</button><button type="button" className="tool-button-secondary" onClick={() => { setValues(EMPTY_RENT); setExample(false); }}>清空</button></div>
...
export function SharedBillTool({ onToast }: { onToast: ShowToast }) {
  ...
  const nameOf = (id: string) => members.find(member => member.id === id)?.name.trim() || `成员 ${members.findIndex(member => member.id === id) + 1}`;
  const summary = result ? `BAYLINK 共享账单分摊\n总额：${usd(result.totalCents)}\n${result.shares.map(share => `${nameOf(share.id)}：${usd(share.amountCents)}（权重 ${share.weight}）`).join('\n')}\n各人金额合计等于账单总额，尾差按权重余数分配。` : '';
  return <div className="tool-form">
    ...
    {error && <p className="tool-error" role="alert">{error}</p>}
    {result ? <div className="tool-split-result" aria-live="polite"><p>每人应承担</p><ul>{result.shares.map(share => <li key={share.id}><span>{nameOf(share.id)}</span><strong>{usd(share.amountCents)}</strong></li>)}</ul><div><span>合计</span><strong>{usd(result.totalCents)}</strong></div></div> : <p className="tool-empty-result">填好总金额，就能看到每人的分摊金额。</p>}
