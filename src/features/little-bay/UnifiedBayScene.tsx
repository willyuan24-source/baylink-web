...
export type UnifiedBaySceneProps={
  ...
  onCitySelect?:(cityId:string)=>void;cityEventCounts?:Readonly<Record<string,number>>;discoveries?:readonly UnifiedSceneDiscovery[];onDiscovery?:(id:string)=>void;onNearbyDiscovery?:(id:string|null)=>void;reducedMotion?:boolean;
};
...
function WorldLabels({props,focusTargetRef}:{props:UnifiedBaySceneProps;focusTargetRef:MutableRefObject<THREE.Vector3>}){
  ...
  useFrame((_,delta)=>{
    ...
    if(level==='region'){
      ...
      for(const city of candidates){if(next.cities.length>=(mobile?6:13))break;const label=props.locale==='en'?city.nameEn:translateText(city.name,props.locale);if(fits(city.position,bayHeight(...city.position)+2.2,Math.min(202,32+label.length*(props.locale==='en'?6.6:11)+(props.cityEventCounts?.[city.id]?24:0))))next.cities.push(city.id);}
    }else{
  ...
  return <group>{visible.regions.map(id=>{const region=BAY_REGIONS.find(item=>item.id===id)!,point=BAY_REGION_CENTERS[region.id];return <Html key={id} center position={[point[0],3,point[1]]} zIndexRange={[2,1]}><button type="button" className="unified-region-label" onPointerDown={event=>event.stopPropagation()} onClick={()=>props.onCitySelect?props.onCitySelect(({sf:'san-francisco',peninsula:'san-mateo','south-bay':'san-jose','east-bay':'oakland'} as const)[region.id]):props.onSelect(BAY_REGION_FOCI[region.id])} aria-label={props.locale==='en'?`Explore ${region.en}`:translateText(`探索${region.zh}`,props.locale)} style={{display:'block',minHeight:44,color:'#285f58',background:'#fff8e8e8',padding:'6px 12px',border:'1px solid #fff9e9',borderRadius:22,fontFamily:'inherit',fontWeight:700,fontSize:size.width<640?11:14,whiteSpace:'nowrap',letterSpacing:'.04em',cursor:'pointer'}}>{props.locale==='en'?region.en:translateText(region.zh,props.locale)}</button></Html>;})}
    ...
    {visible.cities.map(id=>{const city=BAY_CITIES.find(item=>item.id===id)!;return <Html key={id} center position={[city.position[0],bayHeight(...city.position)+2.2,city.position[1]]} zIndexRange={[3,2]}><button type="button" className="unified-city-label" onPointerDown={event=>event.stopPropagation()} onClick={()=>props.onCitySelect?.(id)} aria-label={props.locale==='en'?`Explore ${city.nameEn}`:translateText(`探索${city.name}`,props.locale)} style={{display:'flex',alignItems:'center',gap:6,minHeight:36,border:'1px solid #f3e6cb',borderRadius:20,padding:'5px 11px',background:'#fff8e9ed',color:'#2a6059',fontSize:size.width<640?11:12,fontFamily:'inherit',fontWeight:700,whiteSpace:'nowrap',boxShadow:'0 3px 12px #234e3620',cursor:'pointer'}}><span style={{width:5,height:5,borderRadius:'50%',background:'#be8b50'}}/>{props.locale==='en'?city.nameEn:translateText(city.name,props.locale)}{!!props.cityEventCounts?.[id]&&<b className="ub-map-city-events" title={props.locale==='en'?'Events this and next month':translateText('本月与下月活动',props.locale)}>{props.cityEventCounts[id]}</b>}</button></Html>;})}
