...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, BookOpen, Camera, CarFront, Check, Compass, Expand, Footprints, Map, Minus, Navigation, Pause, Play, Plus, RotateCcw, Search, Sparkles, Stamp, Sun, X } from 'lucide-react';
...
function PlaceSearch({selectedKey,onSelect,onClose}:{selectedKey:string|null;onSelect:(key:string)=>void;onClose:()=>void}) {
  ...
  return <div className="ub-overlay" onPointerDown={e=>e.stopPropagation()} onClick={e=>{if(e.target===e.currentTarget)onClose();}}><section className="ub-search-panel" role="dialog" aria-modal="true" aria-labelledby={title} ref={panel} onKeyDown={keys}>
    ...
    <p className="ub-result-count">{items.length} {t('处发现 · 点选只浏览，不会移动角色','places · Previewing does not move BAYBAY')}</p>
    <div className="ub-search-results">{items.map(place=><button key={place.key} onClick={()=>onSelect(place.key)} aria-pressed={selectedKey===place.key}><span>{place.region==='sf'?'⌂':place.region==='peninsula'?'❀':place.region==='south-bay'?'✦':'♧'}</span><div><strong>{t(place.title,place.titleEn)}</strong><small>{t(BAY_REGIONS.find(r=>r.id===place.region)!.zh,BAY_REGIONS.find(r=>r.id===place.region)!.en)}</small></div><ArrowRight size={17}/></button>)}{!items.length&&<p>{t('没有找到，试试景点的英文名称。','No matches. Try another place name.')}</p>}</div>
  </section></div>;
}
...
export default function UnifiedBayExplorer({date,ownerId,journey,persistent,regionFocus,onRegionChange,onVisit,onAddPlace,addedPlaceIds,onAsk,onShowList,onOpenTown}:UnifiedBayExplorerProps){
  ...
  const overlay=search||journal||sfJournal||!!panel;
  ...
  const select=useCallback((key:string)=>{release();setSelectedKey(key);setMode('overview');setAutoTravel(false);setSearch(false);setCameraOpen(false);setFocusCommand({id:++sequence.current,key});},[release,setSelectedKey,setMode,setAutoTravel,setSearch,setCameraOpen,setFocusCommand]);
  const overview=()=>{release();setMode('overview');setAutoTravel(false);setSelectedKey(null);setNotice('');setCameraOpen(false);setFocusCommand({id:++sequence.current,overview:true});};
  ...
  const quickTravel=(key:string)=>{
    ...
    positionRef.current=point;setPosition(point);setOnWater(false);setRemaining(0);
  ...
  const navigate=(key:string)=>{
    ...
    release();setSelectedKey(key);setPanelKey(null);setSearch(false);setJournal(false);setSfJournal(false);setMode('drive');setRunning(true);setAutoTravel(false);setRemaining(route.distance);
  ...
  useEffect(()=>{setJournal(false);setSfJournal(false);},[ownerId]);
  useEffect(()=>{
  ...
  return <div className="ub-explorer"><section ref={stage} data-unified-bay-stage className={`sf-stage ub-stage${expanded?' is-expanded':''}${touch?' has-touch-controls':''}`} aria-label={t('连续湾区 3D 世界','Continuous Bay Area 3D world')}>
    <div className="ub-canvas"><BaySceneBoundary onError={sceneError} fallback={<div className="ub-loading"><Map size={32}/><p>{t('当前设备无法打开 3D，景点照片和攻略仍可查看。','3D is unavailable on this device. Place photos and guides are still available.')}</p><button onClick={()=>setSearch(true)}>{t('打开全部地点','Browse all places')}</button></div>}><Suspense fallback={null}><Scene mode={mode} running={live} golden={golden} input={input} selectedKey={selectedKey} focusCommand={focusCommand} spawnCommand={spawnCommand} navigation={navigation} autoTravel={autoTravel} locale={locale==='en'?'en':'zh'} visitedKeys={Object.keys(journey.visits)} cameraCommand={cameraCommand} onSelect={select} onArrival={arrived} onVisit={visited} onProgress={progress} onNavigationComplete={completed} onManualInput={manual} onReady={sceneReady} onError={sceneError}/></Suspense></BaySceneBoundary></div>
    {!ready&&!failed&&<div className="ub-loading" role="status"><Compass size={30}/><strong>{t('正在铺开一整片湾区…','Unfolding the whole bay…')}</strong></div>}
    <nav className="ub-topbar" aria-label={t('全湾区游戏导航','Bay world controls')}><div><button className="ub-search-toggle" onClick={()=>setSearch(true)}><Search size={18}/><span>{t('想去哪里','Find a place')}</span></button><button onClick={overview} aria-label={t('全湾鸟瞰','Whole bay overview')} title={t('全湾鸟瞰','Whole bay overview')}><Map size={19}/></button><button onClick={()=>setJournal(true)} aria-label={t('打开湾区任务本','Open Bay adventure journal')}><Sparkles size={18}/><span>{t('任务','Quests')}</span></button></div><div><button aria-label={t('切换光线','Change lighting')} aria-pressed={golden} onClick={()=>setGolden(v=>!v)}><Sun size={17}/></button><button aria-label={running?t('暂停世界','Pause world'):t('继续世界','Resume world')} onClick={()=>setRunning(v=>!v)}>{running?<Pause size={17}/>:<Play size={17}/>}</button><button aria-label={expanded?t('退出全屏','Exit fullscreen'):t('全屏探索','Explore fullscreen')} onClick={()=>setExpanded(v=>!v)}>{expanded?<X size={19}/>:<Expand size={18}/>}</button></div></nav>
    <div className="ub-heading"><small>BAYBAY’S OPEN BAY</small><h2>{mode==='overview'&&!selected?t('一片海湾，连着每一站。','One bay. Every little adventure.'):selected&&mode==='overview'?t(selected.title,selected.titleEn):t(BAY_REGIONS.find(r=>r.id===currentRegion)!.zh,BAY_REGIONS.find(r=>r.id===currentRegion)!.en)}</h2><span>{onWater?t('渡轮航行中','Sailing across the bay'):mode==='overview'?t('拖动环顾 · 选择景点出发','Drag to explore · Choose a place'):mode==='drive'?t('自由驾驶','Free driving'):t('跟 BAYBAY 漫步','Walking with BAYBAY')} · {Object.keys(journey.visits).length}/68</span></div>
    ...
    {shown&&!overlay&&!cameraOpen&&<aside className="ub-place-card" aria-label={t('当前景点','Current attraction')}><div><small>{mode==='overview'?t('目的地预览','DESTINATION'):t('你来到了','YOU ARE HERE')}</small><strong>{t(shown.title,shown.titleEn)}</strong></div><div><button onClick={()=>setPanelKey(shown.key)}><Camera size={16}/>{t('照片攻略','Photos & guide')}</button>{mode==='overview'?<><button onClick={()=>navigate(shown.key)}><Navigation size={16}/>{t('导航','Route')}</button><button onClick={()=>quickTravel(shown.key)}><Footprints size={16}/>{t('快速前往','Quick travel')}</button></>:<>{sfNear&&SF_EXPLORATION_STOP_BY_ID[sfNear]&&<button onClick={()=>setSfJournal(true)}><Stamp size={16}/>{t('小彩蛋','Discover')}</button>}{adventure.nextKey===physicalNear&&<button className="is-primary" onClick={()=>setJournal(true)}><Check size={16}/>{t('完成这一站','Check in here')}</button>}</>}</div></aside>}
    ...
    <div className="ub-hint">{mode==='overview'?t('一个连续世界 · 桥梁连接海湾两岸','One continuous world · Bridges connect the bay'):touch?t('摇杆移动 · 拖动转视角 · 双指缩放','Joystick to move · Drag to orbit · Pinch to zoom'):t('WASD / 方向键 · 点击地面移动 · 拖动转视角','WASD / arrows · Click ground to move · Drag to orbit')}</div>
    {search&&<PlaceSearch selectedKey={selectedKey} onSelect={select} onClose={()=>setSearch(false)}/>}
    {panel&&<UnifiedPlacePanel key={panel.key} place={panel} locale={locale} date={date} addedPlaceIds={addedPlaceIds} onAddPlace={onAddPlace} onAsk={onAsk?ask:undefined} onClose={()=>setPanelKey(null)} onNavigate={navigate}/>}
