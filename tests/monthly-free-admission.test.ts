...
const event = (id: string) => {
  const item = MONTHLY_EVENTS.find(candidate => candidate.id === id);
  assert.ok(item, id);
  return item;
};
...
test('free admission results include Newark Days and Petaluma while retaining paid-extra disclosures', () => {
  ...
  assert.ok(filterMonthlyEvents(MONTHLY_EVENTS, { cost: 'free', date: 'weekend', region: 'east-bay' }, '2026-09-15').includes(newark));
...
test('free admission does not include conditional-age or resident exemptions and still respects expiry', () => {
  ...
  assert.ok(filterMonthlyEvents(MONTHLY_EVENTS, { cost: 'free', includeEnded: true }, '2026-09-21').includes(newark));
});
