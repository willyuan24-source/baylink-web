...
import { getCategoryFromSlug, getSlugFromCategory, isKnownAppPath, tabFromPathname } from '../src/routing';

test('unknown routes and inherited object properties cannot become indexable categories', () => {
  for (const path of ['/not-real', '/category/unknown', '/category/constructor', '/category/__proto__', '/posts/id/extra', '/users/id/extra', '/play/unknown']) assert.equal(isKnownAppPath(path), false, path);
