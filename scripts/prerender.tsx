...
for (const [slug, category] of Object.entries(SLUG_TO_CATEGORY)) {
  await renderPage({ title: `${category}｜湾区本地信息 · BAYLINK`, description: `浏览湾区${category}信息，按地区查找本地资源和邻里需求。请联系发布者确认信息仍有效。`, path: `/category/${slug}` }, (
    <section className="px-5 py-8"><h1 className="text-2xl font-bold">湾区{category}信息</h1><p className="mt-3">浏览本地资源和邻里需求，联系前请确认地点、价格和时间。</p><a className="mt-5 inline-block text-baylink-green underline" href="/guides">先看相关生活指南</a><p className="mt-3 text-sm text-baylink-muted">最新帖子和地区筛选会在页面加载后显示。</p></section>
  ));
}
await renderPage({ title: '编辑推荐｜BAYLINK', description: '查看 BAYLINK 选择展示的本地信息。推荐不构成交易担保，请联系发布者核实。', path: '/recommend' }, <section className="px-5 py-8"><h1 className="text-2xl font-bold">编辑推荐</h1><p className="mt-3">查看平台选择展示的本地信息。联系前请确认服务范围、价格和信息是否仍有效。</p><a href="/" className="mt-5 inline-block text-baylink-green underline">浏览全部本地信息</a></section>);
